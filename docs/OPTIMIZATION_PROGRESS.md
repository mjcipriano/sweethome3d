...
- Updated: 2026-06-12
- Checkpoint branch: `perf/java3d-attrbin-hash`
- Target: eliminate the dominant interactive-3D CPU hotspot
  (`RenderBin.findAttributeBin`/`findTextureBin` bin-list scans) by declaring
  Sweet Home 3D's writable appearance capabilities infrequently changed.
- Reference workload:
...
| 3D render-bin matching root cause (D-render / F1 resolved at app level) | The long-standing `RenderBin.findAttributeBin` hotspot was finally root-caused: Java 3D marks every capability set with `setCapability` as "frequently changed" by default (`capabilityIsFrequentBits = ~0L`), and Sweet Home 3D sets `ALLOW_VISIBLE_WRITE` on a fresh `RenderingAttributes` per shape (plus texture/material/transparency write capabilities per appearance). A changedFrequent component forces `AttributeBin.equals`/`TextureBin.equals` into identity matching, and with `j3d.allowSoleUser` off (the default) `bin.app` is always null, so **no bin ever matches**: every render-atom insertion scans the entire bin list, fails, and appends a new bin - O(n^2) on camera moves, and the explanation for the earlier failed "share one RenderingAttributes" experiment. Fix: `Object3DBranch.setCapabilityInfrequent` (setCapability + `clearCapabilityIsFrequent`) applied to all 33 writable appearance-component capability sites in `j3d` and `HomeComponent3D`. Visibility/material/texture writes still work - they are user-action frequency, which is what the hint declares | On the 435-piece reference home (WSLg D3D12, GTX 1660 Ti, 15 s warm-up + 30 s measure): frames produced rose from ~50 to ~93 median per 30 s and peak FPS from ~28 to ~50; `findAttributeBin`/`findTextureBin` disappeared from the JFR profile entirely (was 699 of 2440 samples top-of-stack). Edit reactions unchanged (move/rotate medians 4/4 ms vs 3/3 baseline, camera 1 ms). `FurnitureVisibility3DTest` (live visibility toggle re-render) passes, core 14/14, GUI 57/59 with only the two documented WSLg focus flakes. New `Object3DBranchCapabilityTest` guards the hint at the creation sites | current branch |
| Native NVIDIA graphics/heap sweep (E2) | Re-stage the current build on native Windows and sweep antialiasing, display lists, and JVM heap on the 435-piece reference home using the GTX 1660 Ti | Display lists remain a consistent loss, confirming `j3d.displaylist=false`. Antialiasing was too noisy to distinguish, so retain the existing speed default. A 1 GB heap repeatedly entered `OutOfMemoryError: GC overhead limit exceeded` while building furniture scene attributes; clean 2 GB runs completed at 9 and 20 FPS, while 4 GB varied widely (6-58 FPS across runs). Run-to-run variance at a fixed config is large on the busy machine, so 2 GB vs 4 GB could not be cleanly separated, but the 2 GB stalls (`fps_min` ~0-1) are GC pauses that more heap reduces. Modernize the heap to adaptive `-XX:MaxRAMPercentage=50.0` in `scripts/package-release.ps1` (the jpackage `--java-options` for every shipped build) and `scripts/run.bat` - that yields >=4 GB on any >=8 GB machine yet never fails to start on small ones - to cut GC pressure, instead of the fixed 2 GB. Separately, the **navigation-proxy prototype was benchmarked and rejected**: swapping furniture to boxes during camera motion gave no FPS gain over full geometry at adequate heap (49-58 vs 56), so it is parked on `perf/3d-navigation-proxy`, not shipped | current branch |
...
| Patch Java 3D's `RenderBin.findAttributeBin` to a hash lookup (F1) | Implemented and benchmarked: a verified value-keyed `HashMap` per `EnvironmentSet` (key snapshots the `RenderingAttributesRetained.equivalent()` fields; identity-matched soleUser/changedFrequent bins excluded; every hit re-verified with the original `equals()`; kill switch `-Dj3d.optimizeAttributeBinLookup=false`), compiled from the JogAmp java3d-core 1.6.2 tag, whose RenderBin/EnvironmentSet/AttributeBin sources are byte-identical to the shipped 1.6.2a jar's 1.6-master branch. It could not help the real workload because the hot queries carry changedFrequent attributes (identity semantics, map ineligible) - that observation led to the actual root cause above. With the capability fix in place the map showed no measurable benefit on interleaved WSLg A/B runs (bin lists are short and value-matched again) | Not shipped: the app-level `setCapabilityInfrequent` fix removes the pathology without maintaining a patched third-party binary. Re-open only if a profile again shows `findAttributeBin`/`findTextureBin` with value-matched bins |
...
**RESOLVED 2026-06-12:** the `RenderBin.findAttributeBin` hotspot turned out to
be fixable at the app level after all - the bins were identity-matched (never
matching) because every `setCapability` call marks the capability "frequently
changed" by default; see the Completed table (`setCapabilityInfrequent`) and the
rejected Java 3D hash-patch experiment. After the fix the JFR profile of the
reference-home camera sweep shows no render-bin frames at all; the remaining
heavy-view minimum (~0-1 FPS facing the high-poly models) is raw triangle
volume, which is the LOD workstream's territory. Do not retry per-piece
`compile()` or geometry merging until immutable shared geometry and mutable
instance capabilities are separated.
