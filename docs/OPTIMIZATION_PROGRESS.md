...
| Re-enable stable WSLg GUI/Java 3D tests | Run each `GL_TEST_EXCLUDES` class individually on WSLg after the Java 3D 1.6 classpath fix, update stale assertions for the default Vertices furniture column, and remove classes that pass reliably from the exclusion list | Re-enabled in both WSLg and CI/Xvfb: `ModelLODGeneratorTest`, `PrintTest`, `UserPreferencesPanelTest`, `PhotoCreationTest`, `IconManagerTest`, `ModelManagerTest`, `HomeControllerTest`, `PlanControllerTest`, and `PackageDependenciesTest`. Still excluded: `PlanComponentTest`, `OBJWriterTest`, `PlanComponentWithFurnitureTest`, `ImportedTextureWizardTest`, `HomeFileRecorderTest`, `ImportedFurnitureWizardTest`, `HomeCameraTest`, `TransferHandlerTest`, `BackgroundImageWizardTest`, `HomeFurniturePanelTest`, `RoomTest`, and `LevelTest`. `OBJWriterTest` and `RoomTest` pass individually but fail in the expanded suite; `ImportedTextureWizardTest`, `BackgroundImageWizardTest`, and `HomeFurniturePanelTest` pass on WSLg but fail under CI/Xvfb, so treat these as environment/order-dependent until isolated | current branch |
