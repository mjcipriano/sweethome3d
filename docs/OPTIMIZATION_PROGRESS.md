...
| Re-enable stable WSLg GUI/Java 3D tests | Run each `GL_TEST_EXCLUDES` class individually on WSLg after the Java 3D 1.6 classpath fix, update stale assertions for the default Vertices furniture column, and remove classes that pass reliably from the exclusion list | `make test-gui` now passes 45 tests across 24 classes. Re-enabled: `ModelLODGeneratorTest`, `PrintTest`, `UserPreferencesPanelTest`, `ImportedTextureWizardTest`, `PhotoCreationTest`, `IconManagerTest`, `ModelManagerTest`, `BackgroundImageWizardTest`, `HomeFurniturePanelTest`, `HomeControllerTest`, `PlanControllerTest`, and `PackageDependenciesTest`. Still excluded: `PlanComponentTest`, `OBJWriterTest`, `PlanComponentWithFurnitureTest`, `HomeFileRecorderTest`, `ImportedFurnitureWizardTest`, `HomeCameraTest`, `TransferHandlerTest`, `RoomTest`, and `LevelTest`. `OBJWriterTest` and `RoomTest` pass individually but fail in the expanded suite, so treat them as order-dependent until isolated | current branch |
