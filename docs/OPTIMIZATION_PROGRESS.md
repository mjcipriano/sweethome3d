...
| Re-enable stable WSLg GUI/Java 3D tests | Run each `GL_TEST_EXCLUDES` class individually on WSLg after the Java 3D 1.6 classpath fix, update stale assertions for the default Vertices furniture column, and remove classes that pass reliably from the exclusion list | Re-enabled in both WSLg and CI/Xvfb: `ModelLODGeneratorTest`, `PrintTest`, `UserPreferencesPanelTest`, `IconManagerTest`, `ModelManagerTest`, `PlanControllerTest`, and `PackageDependenciesTest`. Still excluded: `PlanComponentTest`, `OBJWriterTest`, `PlanComponentWithFurnitureTest`, `ImportedTextureWizardTest`, `PhotoCreationTest`, `HomeFileRecorderTest`, `ImportedFurnitureWizardTest`, `HomeCameraTest`, `TransferHandlerTest`, `BackgroundImageWizardTest`, `HomeFurniturePanelTest`, `RoomTest`, `HomeControllerTest`, and `LevelTest`. `HomeControllerTest` has its stale Vertices-column assertion fixed on this branch, but it still fails later in the combined CI/Xvfb run on an order-sensitive furniture-group geometry assertion. `OBJWriterTest` and `RoomTest` pass individually but fail in the expanded suite; `ImportedTextureWizardTest`, `PhotoCreationTest`, `BackgroundImageWizardTest`, and `HomeFurniturePanelTest` pass on WSLg but fail under CI/Xvfb or CI's media stack, so treat these as environment/order-dependent until isolated | current branch |
